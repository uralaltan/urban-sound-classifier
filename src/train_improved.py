...
class AdvancedSpecAugment:
    """Advanced SpecAugment with configurable parameters"""
...
class WarmupCosineScheduler:
    ...
    def __init__(self, optimizer, warmup_epochs, total_epochs, base_lr, min_lr=1e-6):
        ...
        self.total_epochs = int(total_epochs)  # Ensure integer
        self.base_lr = float(base_lr)  # Ensure float
        self.min_lr = float(min_lr)  # Ensure float
...
def main(cfg_path, pretrained_path=None):
    ...
    # Training loop
    best_acc = 0.0
    best_models = []  # Store top-k models
